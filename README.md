...
- 测验（`/api/quiz`）AI 出题闭环：按知识点用 LLM 生成单选题（JSON 模式 + 失败自动重试），不可用时兜底内置 mock；交卷由服务端判分，错题写入 `WrongbookItem` 持久化；取测验响应不下发正确答案，避免交卷前泄题。
...
- Quiz 出题暂基于知识点标题/说明，尚未接入资料 chunks/RAG 提升相关性。
